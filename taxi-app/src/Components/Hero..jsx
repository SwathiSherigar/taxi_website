...
export default function HeroTaxi() {
  ...
  return (
    <section className="relative flex justify-center items-center h-[85vh] overflow-hidden" id="home">
        ...
        {images.map((image, index) => (
          index === currentImage && (
            <motion.div
              ...
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }} // Try removing this if it's hiding too soon
              transition={{ duration: 1 }}
              className="absolute inset-0 w-full h-full bg-cover bg-center"
              style={{ backgroundImage: `url(${image})` }} // Debug this path in console
            />
          )
        ))}
      </AnimatePresence>
      ...
      <div className="relative z-10 text-center px-4">
        ...
        <motion.p
          className="text-lg font-light text-gray-300 lg:text-xl mb-8 sm:px-4 lg:px-24"
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
